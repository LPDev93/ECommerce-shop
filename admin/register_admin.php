    ...
    if ($select_admin->rowCount() > 0) {
        ...
        if ($pass != $cpass) {
            ...
            $insert_admin = $conn->prepare(
                "INSERT 
                INTO `admins`(name, password)
                VALUES(?, ?)"
            );
            $insert_admin->execute([$name, $pass]);
            $message[] = '¡Usuario administrativo registrado con éxito!';
        }
    }
}
...
<body>
    ...
    <section class="form-container">
        <form action="" method="POST">
            ...
            <input type="password" name="cpass" maxlength="20" placeholder="Confirmar contraseña" class="box" required oninput="this.value = this.value.replace(/\s/g, '')">
